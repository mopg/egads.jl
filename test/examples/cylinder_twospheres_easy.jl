...
datasph = [0.0, 0.0,  width/2.0, rad]
(ebody3, status) = EG_makeSolidBody(context, SPHERE, datasph)
if (status != EGADS_SUCCESS) cleanup(status, context) end

# boolean operations
(emodel2, status) = EG_solidBoolean(ebody1, ebody2, FUSION)
if (status != EGADS_SUCCESS) cleanup(status, context) end

# add second sphere
(ebody4, status) = getBodyFromModel( emodel2 )
if (status != EGADS_SUCCESS) cleanup(status, context) end
(emodel, status) = EG_solidBoolean(ebody4, ebody3, FUSION)
if (status != EGADS_SUCCESS) cleanup(status, context) end
